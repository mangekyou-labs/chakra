...
## 8. Key differences from Arc/Chakra

- **EVM, not Arc**: tx hash, not XDR; `eth_sendTransaction`, not `submit_tx`
- **Permit2**: EIP-712 typed data for gasless approvals; `typed_data: null` when already approved
- **No `prefer_arc`**: Arc has no Arc AMMs; all routes are EVM-native
