...
| Component | Role |
|-----------|------|
| `aggregator.round_trip_swap` | base → bridge → base in one Arc invocation |
| `vault.execute_round_trip` | vault → caller → aggregator → vault (pooled capital; callers only need Arc for fees) |
...
ARB_AGGREGATOR_CONTRACT=C...
ARB_CALLER_SECRETS=...   # each only needs native Arc for fees
```
...
2. `deposit` trading principal (e.g. Arc) into vault
