...
|-------|--------|-----------|
| Worker / router / API | Rust (port LumAgg crates) | Keep proven PathFinder, local math, Brent splitter |
...
CLMM hops with `coverage.is_complete=false` are skipped by QuoteEngine (same policy as LumAgg). QuoteEngine also skips pools whose factory is not in `chakra:factories`.
...
Per-hop `minAmountOut` is **0**. Only the route-total `minimum_output` is enforced on-chain (LumAgg policy).
...
**Dropped vs LumAgg:** `/orders*`, `/dca*`, `/stats`, prices/sparklines, `prefer_soroban`, partner `X-API-Key`.
...
Validation (port LumAgg `build_tx` checks, plus EVM):
...
- `QUOTE_RPC_HYDRATE_ENABLED=false` by default (LumAgg default). Emergency RPC hydrate only.
...
Focused swap app. Rewrite the existing Next.js shell in place (drop Freighter / trustlines / limit / DCA / portfolio).
...
- `crates/dex-adapters` — replace Stellar adapters with EVM: `chakra_xyk`, `chakra_stable`, `chakra_clmm`, plus discovery
...
**Drop from v1 workspace build** (may remain on `main` until rewrite deletes them on this branch): Soroban `contracts/*`, `crates/arbitrage`, vault, order-escrow, limit-keeper, analytics-indexer, classic_dex, comet weighted, `prefer_soroban`.
...
|----------|--------|--------------|-----|
| Architecture | Port LumAgg Rust router + Redis | App Kit wrap; TS-only router | Proven split math; App Kit is closed; TS rewrite would re-bug the optimizer |
...
| Split policy | LumAgg thresholds + Brent; **reject split plans reusing a pool** | Always split; always single | Avoid dust splits; only split when impact or competitiveness warrants; never double-count one pool's liquidity |
| Hop mins | 0 per hop; total `minAmountOut` only | Per-hop mins | Same as LumAgg; rounding across hops otherwise false-reverts |
| `build_tx` | Encode + validate client `sub_routes` | Re-quote on build | Same as LumAgg; UI owns the quote the trader accepted |
...
| CLMM incomplete ticks | Skip hop | RPC hydrate ticks on quote | Same as LumAgg; keep quotes correct |
...
| Rate limit | 10 req/s/IP; health/ready exempt | Partner keys; global limiter | Port LumAgg anonymous limit; partner keys are a non-goal |
| Repo | Rewrite in place on `feature-chakra` | New repo; dual-chain | `main` stays Stellar until merge |
...
- Cold pools keep last Redis value until overwritten (LumAgg principle).
...
| SC-6 / SC-9 SDK walkthrough | SDK + dropped vs LumAgg list |
...
| `build_tx` | Re-quote; encode client routes | **Encode + validate** (LumAgg) |
