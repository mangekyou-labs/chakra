...
### T10.3 Five-check venue verification (Done 2026-08-30)
- `crates/dex-adapters/src/evm_fetch.rs`: added `token0_selector()`, `token1_selector()`, `path_usd_selector()`, and `verify_canonical_token_endpoints()` to verify `token0`/`token1` against catalog token pairs on pool contracts and `pathUSD` against canonical USDC on the Presto hub.
- `crates/market-data-worker/src/evm_watcher.rs`: `discover_once` enforces the 5-check venue verification: (1) bytecode presence at factory and pool addresses, (2) canonical token endpoints (`verify_canonical_token_endpoints`), (3) factory membership (`factory_has_*` pool mapping / hub address match), (4) nonzero reserves/balances (`reserve_a > 0 && reserve_b > 0` / `liquidity > 0`), and (5) probe quote verification (`quote(..., 1_000_000) > 0`). Seed factories only enter `verified_factories` if $\ge 1$ candidate pool passes all 5 checks.
- Tests: `venue_five_checks_all_pass_published`, `venue_five_checks_bytecode_fails_skipped`, `venue_five_checks_canonical_endpoints_fail_skipped`, `venue_five_checks_factory_membership_fails_skipped`, `venue_five_checks_nonzero_reserves_fail_skipped`, `venue_five_checks_probe_quote_fails_skipped`, and `presto_five_checks_path_usd_endpoint_fails_skipped`.

### T10.4 Operator script allowlist + fixture chain (Done 2026-08-30)
- `scripts/arc-operator.sh`: added allowlist enforcement permitting ONLY `DeployAggregator.s.sol` on Arc operator workflow (5042002), rejecting all fixture scripts before invoking forge.
