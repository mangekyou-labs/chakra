...
Kept workspace members: `lumagg-config`, `market-snapshot`, `market-data-worker`, `router-engine`, `dex-adapters`, `api-server`, `lumagg-alerts`, `sdk`.

Excluded from default build: Soroban `contracts/*`, `arbitrage`, `limit-keeper`, `analytics-indexer`, `lumagg-swap-api`.
...
- Changed Redis key prefixes from `lumagg:` to `chakra:` in `pool_state_store.rs` and default `redis.channel`.
- Removed indexer/stats/orders/swaps/arbitrage/xlm_price from `api-server` `lib.rs` so the crate compiles without `analytics-indexer`.
- Stellar adapters still compile (T3.2 will replace them). Default workspace no longer builds WASM contracts.
...
- **Key constants** (`crates/market-snapshot`):
  - `store.rs`: `DEFAULT_REDIS_EVENTS_CHANNEL = "chakra:snapshot:events"`; new `SNAPSHOT_CURRENT_KEY = "chakra:snapshot:current"`; `RedisSnapshotStore` key prefix flipped `lumagg:snapshot` → `chakra:snapshot`. Versioned payloads remain `chakra:snapshot:data:{version}` (+ `:meta:{version}` + `:versions` index) so `current`/`events`/`versions` cannot collide. **Deviation vs the design table's `chakra:snapshot:{version}`** — documented here and in the planning Phase 6 summary.
...
- **`PoolStateStore` trait:** `set_stable_batch`/`fetch_stable` and `set_factories`/`fetch_factories` implemented on `MemoryPoolStateStore` (stable map + factories vec, `pool_count()` helper) and `RedisPoolStateStore` (set_ex/mget + JSON `chakra:factories`). `publish_pool_state` writes stable (currently empty — Aquarius/Comet types kept until T3.2 adapter replacement). `RedisPoolStateStore::snapshot_exists` uses `SNAPSHOT_CURRENT_KEY`.
...
- **Defaults flipped:** `DEFAULT_REDIS_EVENTS_CHANNEL` now `chakra:snapshot:events` — consumed by api-server `AppConfig`, worker `WorkerConfig` (both read `SNAPSHOT_REDIS_CHANNEL` override), and api-server tests updated. Remaining `lumagg:` strings live only in Stellar-era bins (`dex-adapters/src/bin/{dump,audit}_ledger_events.rs`), untouched.
...
- `worker.rs` — `WorkerMode::{Arc,Stellar}`; `from_env` defaults to **Arc** (Stellar loop only when `RPC_URL`/`SNAPSHOT_REDIS_URL` set and no `CHAKRA_*`); `run()` dispatches to `evm_watcher::run_arc`.
...
- **EVM hop dispatch in `quote_path`:** `chakra-stable` → `local_stable_quote` (uses `evm_quote_math::stable_quote`, token index from hydrated `token_a/b`, **no** `MIN_XYK_RESERVE_STROOPS` on stable balances, impact = `(spot - out)*10000/spot` vs the 1:1 spot — the 4 bps venue fee is always paid); `chakra-xyk` → `local_evm_xyk_quote` (`evm_quote_math::xyk_quote` 997/1000 + integer `price_impact_bps` on hydrated reserves, dust-reserve guard kept). Stellar generic 9970/10000 / Aquarius / Comet paths are untouched.
...
- **`PathFinderConfig::default()` fixed for Chakra:** `max_hops = 3` (unchanged), `bridge_tokens = [TokenId::Contract { address: USDC_ERC20 (0x3600…0000) }]` — the old default bridged **XLM Native + Classic USDC**, which was wrong on Arc. (The BFS itself explores all edges; the bridge list is compatibility — asserted by the default-config test.)
...
- **OpenAPI** (`docs/openapi.yaml`) — retitled Chakra; envelope schemas + error codes; dropped all Stellar paths (`/orders*`, `/stats`, `/prices*`, `/submit_tx`, `/tx_status`, `/account`, `/classic_asset`, `/ledger/latest`, `prefer_soroban`); `/ready` predicate text; balances never-sum; **two quote examples**: (a) honest 100 USDC single `chakra-stable`, (b) 70/30 split labeled **illustrative / not current engine output** (doc deviation, see below). `docs/api-reference.md` rewritten.
- **Legacy removed** — Stellar-era integration tests (`snapshot_quote_test`, `redis_snapshot_smoke_test`, `build_tx_simulate_test`, `decode_user_tx`) + `verify_split_quote` example (referenced deleted Stellar handler symbols). Stellar modules still compile but are not constructed on the Arc path.
...
- `src/app/providers.tsx` — `WagmiProvider` + `QueryClientProvider` (client-only; `QueryClient` in a `useState` initializer). Stellar dynamic `WalletProviderInner` dropped.
- `HeaderWallet` — Connect / `0x…` truncate / wrong-chain amber dot + "Switch to Arc Testnet" CTA / "Gas: USDC" label. `WalletButton` (Freighter) deleted (was never imported).
- `next.config.ts` — `transpilePackages: ['@creit.tech/stellar-wallets-kit']` removed (webpack fallbacks kept).
...
- `packages/sdk/src/index.ts` rewritten: `LumAggClient` → `ChakraClient`; Stellar surface dropped (orders/DCA/stats/submit/XDR/trustlines/prices/account/classic_asset/ledger).
...
- `quote(params)` — query `token_in`, `token_out`, `amount_in`, `slippage_bps` (via `slippageToBps(slippage?, slippageBps?)`; `slippage` percent → `Math.round(slippage * 100)`), optional `max_hops`/`max_splits`. Never sends `prefer_soroban` or percent `slippage`. Maps response to camelCase (`priceImpactBps`, `protocolFeeBps`, `fractionBps`, `subRoutes`).
...
- **Deleted Stellar-only frontend files** (see `Changed files` below). `lib/balance.ts`, `lib/routeDisplay.ts`, `lib/swap-selection.ts` removed in favor of `lib/decimals.ts`.
...
- `src/lib/swap-settings.ts` — storage key `chakra:swap-settings` (old `lumagg.swapSettings` unread), `DEFAULT_SWAP_SETTINGS.slippage = 0.5`, `maxHops 3`, `maxSplits 5`.
...
- `src/components/BuildTxCodeSample.tsx` — Chakra quote → build_tx sample (no Stellar imports); `docs/ApiReference.tsx` + `docs/page.tsx` rewritten to the Chakra surface (`user`, `slippage_bps`, no deleted endpoints).
...
- `crates/lumagg-config/src/aggregator.rs`
...
- `crates/api-server/` (T4.3 rewrite): `src/{lib.rs,main.rs,config.rs,state.rs,rate_limit.rs,handlers.rs}` rewritten; `src/{envelope.rs,catalog.rs,hydrate.rs,evm_balances.rs}` new; `src/{abi.rs,build_tx.rs}` new (T4.4); `src/{pool_hydrate.rs,snapshot_loader.rs,soroban_prepare.rs,orders.rs,stats.rs,swaps.rs,price_*.rs,prices.rs,arbitrage.rs,xlm_price.rs}` remain on disk but are **no longer wired** (dead modules; Stellar surface dropped); `tests/chakra_rest_test.rs` + `tests/chakra_build_tx_test.rs` new; `tests/{redis_snapshot_smoke_test,snapshot_quote_test,build_tx_simulate_test,decode_user_tx}.rs` + `examples/verify_split_quote.rs` **removed** (Stellar-era)
...
- Dex adapters still Stellar-named; compiling keep-set, not yet EVM (T3.2 — **T3.3 supersedes this for the worker path**: the Arc path never constructs Stellar adapters, though the crates still compile).
...
- **T3.3: pool index + watch lists accept EVM `0x` addresses** (lowercased) in addition to Stellar `C…` ids — the Stellar `is_contract_address` guard alone would have dropped every Arc pool.
- **T3.3: `spawn_arc_pipeline` constructs never-contacted Stellar adapter stubs** (cheap constructors, no I/O) so the single fetch pipeline serves both paths without Option-wrapping its whole context; no Stellar task is ever enqueued on the Arc path.
...
- **T6.1: Stellar wallet dep removal pulled the Stellar-UI deletions forward.** Removing `@stellar/*` deps (T6.1) forces every importer handled in the same pass, so the T6.2 "delete Stellar-only files" list landed with T6.1's green pass; T6.2's own red/green gate is the unit behaviors (decimals port, slippage 0.5, bps formatters, debounce, native-encoding filter). Both boxes are flipped with the full evidence.
...
- **T6.2: `docs/ApiReference.tsx` + `docs/page.tsx` rewritten beyond the plan's file list** — they referenced deleted Stellar endpoints (`/swaps`, `/prices`, `/submit_tx`, `user_public_key`) and would have sent dead calls from the try-it UI; rewritten to the shipped Chakra surface (`user`, `slippage_bps`, `/health`, `/ready`, `/tokens`, `/balances`, `/quote`, `/build_tx`).
...
- Full workspace `cargo test --workspace` still fails to compile the legacy Stellar bins/tests (`dex-adapters/src/bin/*`, `tests/aquarius_3token_stableswap.rs`) — pre-existing excluded targets (Arc compile strip; the documented gate is the kept-crate set above).
...
- `cargo test --workspace` (T3.3, 2026-08-25): **all suites 0 failed** (market-snapshot 35, dex-adapters 116, market-data-worker 23, router-engine 29, api-server 5, lumagg-* …).
...
- **Clean-Clone SDK Walkthrough (T7.2 / SC-6 / SC-9):**
  ...
  - Installed `@lumagg/sdk` dependencies, compiled with TypeScript (`npm run build`).
