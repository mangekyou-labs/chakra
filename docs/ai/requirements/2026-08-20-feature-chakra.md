...
Sources: this repo’s Chakra architecture (`README.md`), SCF #44 grant surface (`https://communityfund.Arc.org/submissions/recFRG56TbGtuXbMt`), Arc agent context (`~/.arc-canteen/context/AGENTS.md`, `circlefin-skills/use-arc.md`, `circlefin-skills/use-usdc.md`, Arc `contract-addresses.md`). Chakra is **not** a Arc dual-chain product and **not** a Circle App Kit wrapper.
...
- Weighted (Arc venue / Balancer-style) pools.
...
- Keep Chakra principles: topology vs state, single Redis writer, event-driven freshness, local quote math, split only when warranted, atomic aggregator execution.
...
|----------|-----------|----------|
| **A. Port Chakra worker/Redis/router/API + Solidity aggregator + focused Next.js UI** | Highest fidelity to proven split math; more port work | **Chosen** |
...
**Validated:** problem, users, goals/non-goals, stories, SC-1…13, Chakra-port approach, hybrid venues, Permit2, 0 fee, EIP-6963, dense terminal, public REST+SDK, rewrite on `feature-chakra`, testnet-only.
