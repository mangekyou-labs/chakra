...
- [x] D1 Benchmark matrix + refresh ([scf-benchmark-results.md](./scf-benchmark-results.md) — **2026-07-21** fair `prefer_arc` run; split + parity rows in “Tranche 1 reviewer highlights”)
- [x] D2 OpenAPI + [integrator-guide.md](./integrator-guide.md) + `prefer_arc`
...
- [x] D3 Swap UI (logos, balance %, explorer) — self-hosted logos via `https://api.Chakra.xyz/logos/` — prod `/tokens` returns logos; letter-avatar `logo_kind` fallbacks
...
- [x] D5 `npm publish @Chakra/sdk` — https://www.npmjs.com/package/@Chakra/sdk (`0.2.0`)
...
- [x] D8 `/api/v1/stats` + https://Chakra.xyz/stats (after deploy)
