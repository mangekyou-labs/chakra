...
| **Public API** | `https://chakra-api-0a5i.onrender.com` | Live (Render Docker) |
...
|---|---|---|---|
| **SC-1** | Multi-venue route discovery across catalog (USDC, EURC, mBTC) | `docs/evidence/chakra-t91-venue-matrix.json` (15 matrix points across 5 pairs) | **PASS** |
...
| **SC-3** | UI critical path (connect, switch Arc, quote legs/impact/fee=0, Permit2, swap) | `docs/evidence/chakra-t98-manual-ux-a11y.json` (UI layout & controls verified; on-chain wallet send pending funded MetaMask) | **UI PASS / Wallet Pending** |
...
| **SC-8** | Venue comparison matrix (≥3 pairs × ≥3 sizes) & split benchmark | `docs/evidence/chakra-t91-venue-matrix.json` & `docs/evidence/chakra-t92-split-benchmark.json` | **PASS** |
...
2. **`docs/evidence/chakra-t91-venue-matrix.json`**: Comprehensive 15-scenario route matrix across 5 pairs and 3 sizes each.
3. **`docs/evidence/chakra-t92-split-benchmark.json`**: Split vs single-path optimization benchmark demonstrating +893.01 bps gain.
4. **`docs/evidence/chakra-t95-quote-latency.json`**: 100-sample latency benchmark demonstrating 23 ms p95 server-side API compute time on deployed revision.
5. **`docs/evidence/chakra-t98-manual-ux-a11y.json`**: Desktop and mobile UI viewport and accessibility compliance audit.
...
- **T2.1–T2.4 (Pool Deepening):** Re-seeding pools to target 200k/10k sizes requires Circle faucet inventory.
