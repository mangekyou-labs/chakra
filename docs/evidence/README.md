...
| **SC-3** | UI critical path (connect, switch Arc, quote legs/impact/fee=0, Permit2, swap) | `docs/evidence/chakra-t98-manual-ux-a11y.json`; **live 2026-08-28** — MetaMask UI swap tx `0xa630da3c842d7613ebbbd4d8f66749892a4e42c510933e0e1c3f4966907ef0dd` (QA wallet `0xc603C3…dCE76`, status 1, block 59292405) | **PASS (Live)** |
| **SC-4** | On-chain atomic split swap (≥2 sub-routes in 1 tx) on Arcscan | **LIVE 2026-08-28** — tx `0x42e85916ade38b87ef0440ef71d8f3330075ecf2a481247dc2ac33376b287fa8` (3 sub-routes: 2× xylo + 1× chakra-stable, `isSplit=1`, status 1, block 59271873, 5e6 USDC→4,674,618 EURC) | **PASS (Live)** |
...
| **SC-7** | Playwright CLI MetaMask test on Arc testnet | `docs/qa-playwright-metamask.md` & `packages/frontend/qa/wallet/swap-critical-path.spec.ts` — **LIVE PASS 2026-08-28** (`swap-critical-path` exit 0, 38.3 s, seeded via gitignored `QA_WALLET_SECRET`; dAppwright `addNetwork`/`confirmNetworkSwitch` broken on MetaMask 13.17 → popup driven directly) | **PASS (Live)** |
