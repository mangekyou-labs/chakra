# Round-trip arbitrage via Chakra aggregator
...
- `user`: bot G-address (`require_auth`); holds Arc/USDC float — **no custodial deposit**
...
One `InvokeHostFunction` per transaction (Arc protocol limit).
...
ARB_BASE_TOKENS=Arc,USDC                  # optional, defaults Arc+USDC
...
ARB_MIN_AMOUNT_IN=100000000               # 10 Arc
ARB_MAX_AMOUNT_IN=180000000000            # 1800 Arc
ARB_OPTIMIZE_AMOUNT=1                     # max(out-in) over sample_count inputs
ARB_SAMPLE_COUNT=8
ARB_MIN_PROFIT=80000                      # default post-fee floor (base units, 7dp)
ARB_MIN_PROFIT_Arc=80000                  # optional per-base floors
ARB_MIN_PROFIT_USDC=30000
ARB_Arc_USDC_PRICE_E7=1800000             # fallback USDC/Arc for fee conversion
ARB_Arc_USDC_PRICE_REFRESH_SECS=60        # live quote refresh; 0 = fallback only

ARB_BUILD_TX=1
ARB_SUBMIT_TX=1
ARB_DRY_RUN=1

cargo run -p arbitrage --bin Chakra-arbitrage-bot
```
