...
**Tech Stack:** Rust (api-server, rusqlite, QuoteEngine), Next.js frontend, `@Chakra/sdk`.
...
| `crates/api-server/src/price_mark.rs` | Quote token→USDC (via Arc fallback) |
...
- Arc SAC: `CAS3J7GYLGXMF6TDJBBYYSE3HQ6BBSMLNUQ34T6TZMYMW2EVH34XOWMA`
...
pub async fn mark_token_usdc(state: &AppState, token: &str) -> Option<(f64, &'static str)> {
    ...
    let Arc_out = quote_one(state, token, Arc_SAC).await?;
    let Arc_usdc = quote_one(state, Arc_SAC, USDC_SAC).await?;
    Some((Arc_out * Arc_usdc, "Arc"))
}

async fn quote_one(state: &AppState, token_in: &str, token_out: &str) -> Option<f64> {
    // Build RouteRequest amount_in=TOKEN_UNITS, prefer_arc=true optional
...
Token list: start from `dex_adapters::common_balance_tokens` / hardcoded priority (Arc, USDC, EURC, AQUA) then pad from `list_tokens` source used by handler (read how `list_tokens` gets ids — reuse that function or `collect_common_balance_token_ids`).
...
| Sampler 600s, whitelist, USDC/Arc mark | 2 |
