# Chakra atomic arbitrage — operator playbook

Tranche 2 deliverable: self-deploy **aggregator + vault + arb scanner** on mainnet.

**Not for retail users.** The vault pools trading capital; bot callers only need Arc for Arc fees.

## Architecture

```text
quote-api (×N) ──► Chakra-arbitrage-bot ──► vault.execute_round_trip ──► aggregator.round_trip_swap ──► DEX pools
                      ▲                    │
...
- Mainnet Arc RPC (local node or gateway)
...
```bash
# deposit Arc (SAC) into vault from ops wallet
Arc contract invoke --id $VAULT -- deposit --from OPS_G --token $Arc_SAC --amount <atomic unitss>
```

Allowlist each bot caller:

```bash
Arc contract invoke --id $VAULT -- add_caller --caller GCALLER...
```
...
| `ARB_AGGREGATOR_CONTRACT`                  | Chakra aggregator                            |
...
| `ARB_BASE_TOKENS`                          | Usually Arc SAC                              |
...
| `ARB_MIN_PROFIT_Arc` / `ARB_MIN_PROFIT_USDC` | Optional per-base floors                     |
| `ARB_Arc_USDC_PRICE_E7` | Fallback USDC units per 1.0 Arc when live quote fails (default `1800000`) |
| `ARB_Arc_USDC_PRICE_REFRESH_SECS` | Refresh live Arc→USDC mark from quote-api (default `60`; `0` = fallback only) |
...
| Signal                             | Where                                        |
| ---------------------------------- | -------------------------------------------- |
| SUCCESS / FAILED txs               | `journalctl -u Chakra-arb`                   |
...
| On-chain volume                    | `Chakra-analytics-indexer --config aggregator.toml status` / `/api/v1/stats` |
| Caller Arc                         | Horizon account balance (fee float only)     |
...
Local quotes can look profitable while Arc simulation loses ~20 bps. Prefer fixing the **quote path** (fresher pool state / venue math / optional on-chain hop validation) over execution-side workarounds. Scanner already size-optimizes even when the 10 Arc probe is flat; post-fee `ARB_MIN_PROFIT` still applies after simulate.
...
```bash
curl -sG https://api.Chakra.xyz/api/v1/quote \
...
```bash
journalctl -u Chakra-arb --since today | grep -E 'arb tx SUCCESS|arb stats summary|quiet window'
```
...
Does **not** run inside `Chakra-arb`. Samples quote-api routes and immediately
compares each hop to on-chain `estimate_swap` / fresh Arc venue reserves (and
...
```bash
systemctl status Chakra-quote-sim-probe.timer
journalctl -u Chakra-quote-sim-probe -n 50 --no-pager
# JSONL archive:
tail -f /opt/Arc-dex-aggregator/logs/quote-sim-probe.jsonl
```
...
`deploy/Chakra-quote-sim-probe.timer` → `OnCalendar=hourly` and
`systemctl daemon-reload && systemctl restart Chakra-quote-sim-probe.timer`.

Manual oneshot: `systemctl start Chakra-quote-sim-probe.service`

## 5. Risk & limits

- **Arb-only vault** — no public withdraw; callers cannot drain in a separate tx.
- **Multi-hop fees** — Arc resource fees scale with legs; gate on **net** profit (`ARB_MIN_PROFIT`).
...
- **Contract upgrades** — WASM upload costs ~20 Arc on mainnet; coordinate before upgrade.
