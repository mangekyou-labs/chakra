...
const AQUARIUS_KEY_PREFIX: &str = "chakra:pool:aquarius";
const COMET_KEY_PREFIX: &str = "chakra:pool:comet";
...
impl MemoryPoolStateStore {
    ...
    pub async fn pool_count(&self) -> usize {
        ...
        let aquarius = self.aquarius.read().await.len();
        let comet = self.comet.read().await.len();
        xyk + stable + clmm + aquarius + comet
    }
    ...
    pub async fn pool_keys(&self) -> Vec<String> {
        ...
        keys.extend(self.aquarius.read().await.keys().cloned());
        keys.extend(self.comet.read().await.keys().cloned());
        keys.sort();
        keys
    }
}
