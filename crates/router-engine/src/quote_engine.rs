...
impl QuoteEngine {
    ...
    async fn quote_path(&self, path: &Path, amount_in: u128, hydration: Option<&QuoteHydration>) -> Option<Quote> {
        ...
        for (i, source) in path.sources.iter().enumerate() {
            ...
            let hop_result = if matches!(source.as_str(), "sushi" | "aquarius_clmm" | "chakra-clmm") {
    ...
    /// hydrated stable balances. Never xy=k math, never the Stellar generic
    /// 9970/10000.
    ...
    fn local_clmm_quote(
        ...
        if !matches!(source, "sushi" | "aquarius_clmm" | "chakra-clmm") {
            return None;
        }
