...
mod tests {
    ...
    fn test_paths_carry_per_hop_dex_type_fee_factory() {
        ...
        // Direct path: xlm → usdc via chakra-stable.
        let direct = graph.find_paths(&xlm(), &usdc(), 4, 100, 0);
        ...
        // Multi-hop: xlm → usdc → eth via stable then xyk.
        let multi = graph.find_paths(&xlm(), &eth(), 4, 100, 0);
