...
/// Which worker loop runs: the Arc pipeline (T3.3+) or the legacy Stellar loop.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WorkerMode {
    /// EVM watcher + poll + fetch pipeline on `CHAKRA_*` env (default).
    Arc,
    /// Legacy Soroban adapter loop (kept compiling; `RPC_URL` env).
    Stellar,
}
...
impl WorkerConfig {
    pub fn from_env() -> Result<Self> {
        ...
        let has_stellar_env = ["RPC_URL", "SNAPSHOT_REDIS_URL"]
            .iter()
            .any(|name| std::env::var(name).is_ok());
        // Default path is Arc; legacy Stellar loop only when its env is set and
        // no CHAKRA_* env is present.
        let mode = if has_chakra_env {
            WorkerMode::Arc
        } else if has_stellar_env {
            WorkerMode::Stellar
        } else {
            WorkerMode::Arc
        };
    ...
    let fetch_pipeline = if config.fetch_pipeline_enabled {
        pool_state_store.as_ref().map(|pool_store| {
            ...
            crate::fetch_pipeline::spawn_fetch_pipeline(
                ...
                pool_store.clone(),
                rpc.clone(),
                None, // Stellar path: no EVM client
                shared.clone(),
                soroswap.clone(),
                aquarius.clone(),
                phoenix.clone(),
...
mod tests {
    ...
    #[test]
    fn worker_mode_keeps_stellar_when_legacy_env_is_set() {
        let _guard = env_lock().lock().unwrap();
        let original_chakra = [
            "CHAKRA_RPC_HTTP",
            "CHAKRA_RPC_WS",
        ...
        std::env::set_var("RPC_URL", "https://soroban-rpc.testnet.stellar.gateway.fm");
        std::env::set_var("SNAPSHOT_REDIS_URL", "redis://127.0.0.1:6399/");

        let config = WorkerConfig::from_env().unwrap();
        assert_eq!(config.mode, WorkerMode::Stellar);
