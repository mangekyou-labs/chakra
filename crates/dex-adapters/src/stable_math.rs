...
//! D^(n+1)/(n^n * prod(x_i)) Used by Arc venue stableswap pools (2-token and
//! 3-token).
//!
...
mod tests {
    ...
    fn test_stable_3token_router_fixture() {
        // Same fixture as Arc venue::three_token_stableswap_matches_router_test_vector
        let pool = StablePool {
    ...
    fn test_stable_3token() {
        // 3-token pool (like Arc/USDC/AQUA)
        let pool = StablePool {
        ...
        // Swap 1 USDC (token1) -> Arc (token0)
        let out = pool.get_dy(1, 0, 1_0000000);
        println!("1 USDC -> {} Arc (atomic unitss)", out);
        // With these reserves, 1 USDC should get roughly 1 Arc worth
