...
//! event ingestion.
//!
//! Accepts both Stellar `C…` contract ids and EVM `0x…` addresses so the same
...
/// Canonical index key: Stellar ids pass through, EVM addresses are lowercased.
...
mod tests {
    ...
    fn indexes_evm_0x_pool_addresses() {
        ...
        // Stellar ids still indexed (dual-mode rebuild).
